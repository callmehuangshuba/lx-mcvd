...
class TCG_NPYDataset(Dataset):
    ...
    def __init__(self, data_path,   # hdf5 file, or directory of hdf5s
        ...
        if os.path.splitext(self.data_path)[-1] == '.npy' or os.path.splitext(self.data_path)[-1] == '.npy':
        ...
        assert len(self.shard_paths) > 0, "npy: Could not find .hdf5 files! Dir: " + self.data_dir + " ; len(self.shard_paths) = " + str(len(self.shard_paths))
        ...
        for one_TC_vid_path in self.shard_paths:
            frames_list_h5=sorted([os.path.join(one_TC_vid_path,x) for x in os.listdir(one_TC_vid_path) if x.endswith('.h5')])
            one_TC_h5_list=[]
            for frame_name in frames_list_h5:
                with h5py.File(frame_name, 'r') as file:  
                    frame=file['Infrared'][:]
                    frame=(frame-frame.mean())/frame.std()
                    one_TC_h5_list.append(frame) 
