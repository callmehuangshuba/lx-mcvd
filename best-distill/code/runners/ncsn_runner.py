...
from evaluation.torchmetrics_wrap import FrechetVideoDistance
...
def conditioning_fn2(config, X, num_frames_pred=0, prob_mask_cond=0.0, prob_mask_future=0.0, conditional=True):
    imsize = config.data.image_size
    if not conditional:
        return X.reshape(len(X), -1, imsize, imsize), None, None
    ...
    future = getattr(config.data, "num_frames_future", 0)
    B, T, V,  H, W = X.shape
    variables1=[X[:, cond:cond+pred, v] for v in range(V)]
    variables2 = [X[:, :cond, v] for v in range(V)]
    # Frames to train on / sample
    pred_frames = torch.cat(variables1, dim=1)

    # Condition (Past)
    cond_frames = torch.cat(variables2, dim=1)
    ...
    return pred_frames, cond_frames, cond_mask   # , future_mask
def conditioning_fn3(config, X, num_frames_pred=0, prob_mask_cond=0.0, prob_mask_future=0.0, conditional=True):
    imsize = config.data.image_size
    if not conditional:
        return X.reshape(len(X), -1, imsize, imsize), None, None
    channel = 4
...
def get_stu_model(config):
    ...
    if arch == 'unetmore':
        from models.better.ncsnpp_more import UNetMore_DDPM # This lets the code run on CPU when 'unetmore' is not used
        return UNetMore_DDPM(config,is_stu=True).to(config.device)#.to(memory_format=torch.channels_last).to(config.device)
    elif arch in ['unetmore3d', 'unetmorepseudo3d']:
        from models.better.ncsnpp_more import UNetMore_DDPM # This lets the code run on CPU when 'unetmore' is not used
        # return UNetMore_DDPM(config).to(memory_format=torch.channels_last_3d).to(config.device) # channels_last_3d doesn't work!
        return UNetMore_DDPM(config,is_stu=True).to(config.device)#.to(memory_format=torch.channels_last).to(config.device)
...
class NCSNRunner():
    def __init__(self, args, config, config_uncond):
        self.valid_mse = torchmetrics.MeanSquaredError().to(torch.device('cuda'))
        self.valid_mae = torchmetrics.MeanAbsoluteError().to(torch.device('cuda'))
        self.test_ssim = torchmetrics.image.StructuralSimilarityIndexMeasure().to(torch.device('cuda'))
    ...
    def train(self):
        ...
        if self.config.data.dataset.upper() == 'FFHQ':
            ...
            test_loader = DataLoader(test_dataset, batch_size=self.config.training.batch_size//2, shuffle=True,
        ...
        scorenet_stu = get_stu_model(self.config)

        scorenet = torch.nn.DataParallel(scorenet)
        scorenet_stu = torch.nn.DataParallel(scorenet_stu)

        
        if scorenet is not None:
            states_teacher =   torch.load(os.path.join("/opt/data/private/mcvd-pytorch/teacher_pths", f'checkpoint_62000.pt'),
                map_location=self.config.device)
            scorenet.load_state_dict(states_teacher[0], strict=False)
            ema_helper = EMAHelper(mu=self.config.model.ema_rate)
            ema_helper.register(scorenet)
            ema_helper.load_state_dict(states_teacher[-1])
            ema_helper.ema(scorenet)
        logging.info(f"Number of parameters: {count_parameters(scorenet_stu)}")
        logging.info(f"Number of trainable parameters: {count_training_parameters(scorenet_stu)}")

        optimizer = get_optimizer(self.config, scorenet_stu.parameters())
        ...
        if self.config.model.ema:
            # 学生的
            ema_helper_stu = EMAHelper(mu=self.config.model.ema_rate)
            ema_helper_stu.register(scorenet_stu)
            
        if self.args.resume_training:
            # states = torch.load(os.path.join(self.args.log_path, 'checkpoint_2000.pt'))
            states = torch.load(os.path.join(self.args.log_path, 'checkpoint.pt'))
            scorenet_stu.load_state_dict(states[0])
            ...
            if self.config.model.ema:
                #学生的
                ema_helper_stu.load_state_dict(states[4])
        ...
        print(scorenet_stu)
        net = scorenet_stu.module if hasattr(scorenet_stu, 'module') else scorenet_stu
        ...
        # Initial samples
        n_init_samples = max(16, self.config.training.batch_size)
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for batch, (x, y) in enumerate(dataloader):
                ...
                X = x[0]
                # Data
                X = X.to(self.config.device).float()
                ...
                era5_data_2d = era5_data_2d.to(self.config.device).float()
                era5_data_2d = data_transform(self.config, era5_data_2d)
                era5_X, era5_cond, _ = conditioning_fn2(self.config, era5_data_2d, num_frames_pred=self.config.data.num_frames,
                                                     prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                     prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                     conditional=conditional)
                era5_data_3d = era5_data_3d.to(self.config.device).float()
                era5_data_3d = data_transform(self.config, era5_data_3d)
                _, era5_cond_3d, _ = conditioning_fn3(self.config, era5_data_3d, num_frames_pred=self.config.data.num_frames,
                                                     prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                     prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                     conditional=conditional)
                # Loss
                itr_start = time.time()
                loss = anneal_dsm_score_estimation(scorenet_stu,scorenet,X, labels=None, cond=cond, cond_mask=cond_mask,era5_cond=era5_cond,era5_cond_3d=era5_cond_3d,
                ...
                # Optimize
                loss.backward()
                grad_norm = torch.nn.utils.clip_grad_norm_(scorenet_stu.parameters(), getattr(self.config.optim, 'grad_clip', np.inf))
                optimizer.step()
                ...
                if self.config.model.ema:
                    ema_helper_stu.update(scorenet_stu)
                ...
                if (step % 1000 == 0 and step != 0) or step % self.config.training.snapshot_freq == 0:
                    states = [
                        scorenet_stu.state_dict(),
                        optimizer.state_dict(),
                        epoch,
                        step,
                    ]
                    if self.config.model.ema:
                        states.append(ema_helper_stu.state_dict())
                ...
                if step == 1 or step % self.config.training.val_freq == 0 or (step % self.config.training.snapshot_freq == 0 or step % self.config.training.sample_freq == 0) and self.config.training.snapshot_sampling:
                    ...
                    if self.config.model.ema:
                        test_scorenet = ema_helper_stu.ema_copy(scorenet_stu)
                    else:
                        test_scorenet = scorenet_stu

                    test_scorenet.eval()

                # Validation
                if step % self.config.training.val_freq == 0:
                    try:
                    ...
                    test_X = test_x[0]
                    test_era5_X = test_x[1]
                    test_era5_3d_X = test_x[2]

                    test_X = test_X.to(self.config.device).float()
                    test_X = data_transform(self.config, test_X)

                    test_era5_X = test_era5_X.to(self.config.device).float()
                    test_era5_X = data_transform(self.config, test_era5_X)

                    test_era5_3d_X = test_era5_3d_X.to(self.config.device).float()
                    test_era5_3d_X = data_transform(self.config, test_era5_3d_X)

                    test_X, test_cond, test_cond_mask = conditioning_fn(self.config, test_X, num_frames_pred=self.config.data.num_frames,
                                                                    prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                                    prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                                    conditional=conditional)
                    _, test_era5_cond, _ = conditioning_fn2(self.config, test_era5_X, num_frames_pred=self.config.data.num_frames,
                                                                        prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                                        prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                                        conditional=conditional)

                    _, test_era5_3d_cond, _ = conditioning_fn3(self.config, test_era5_3d_X, num_frames_pred=self.config.data.num_frames,
                                                                        prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                                        prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                                        conditional=conditional)
                    with torch.no_grad():
                        test_dsm_loss = anneal_dsm_score_estimation(test_scorenet, scorenet,test_X, labels=None, cond=test_cond, cond_mask=test_cond_mask,era5_cond=test_era5_cond,era5_cond_3d = test_era5_3d_cond,
                                                                    loss_type=getattr(self.config.training, 'loss_type', 'a'),
                                                                    gamma=getattr(self.config.model, 'gamma', False),
                ...
                if (step % self.config.training.snapshot_freq == 0 or step % self.config.training.sample_freq == 0) and self.config.training.snapshot_sampling:
                    ...
                    if conditional:
                        try:
                            ...
                            test_x, test_y = next(test_iter)
                        test_X = test_x[0]
                        test_X = test_X[:len(init_samples)].to(self.config.device).float()
                        ...
                        test_X, test_cond, test_cond_mask = conditioning_fn(self.config, test_X, num_frames_pred=self.config.data.num_frames,
                                                                            ...
                                                                            prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                                            conditional=conditional)
                        test_X_era5 = test_x[1]
                        test_X_era5 = test_X_era5[:len(init_samples)].to(self.config.device).float() # len(init_samples) 表示 init_samples 的第一个维度的大小，即 批量大小（batch size）

                        test_X_era5 = data_transform(self.config, test_X_era5)
                        _, test_cond_era5, _ = conditioning_fn2(self.config, test_X_era5, num_frames_pred=self.config.data.num_frames,
                                                    prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                    prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                    conditional=conditional)

                        test_X_era5_3d = test_x[2]
                        test_X_era5_3d = test_X_era5_3d[:len(init_samples)].to(self.config.device).float() # len(init_samples) 表示 init_samples 的第一个维度的大小，即 批量大小（batch size）

                        test_X_era5_3d = data_transform(self.config, test_X_era5_3d)
                        _, test_cond_era5_3d, _ = conditioning_fn3(self.config, test_X_era5_3d, num_frames_pred=self.config.data.num_frames,
                                                    prob_mask_cond=getattr(self.config.data, 'prob_mask_cond', 0.0),
                                                    prob_mask_future=getattr(self.config.data, 'prob_mask_future', 0.0),
                                                    conditional=conditional)      
                                                   
                    all_samples = sampler(init_samples, test_scorenet, cond=test_cond, cond_mask=test_cond_mask,cond_era5=test_cond_era5,cond_era5_3d=test_cond_era5_3d,
                    ...
                    # pred=self.decode_first_stage(pred.to(self.config.device)).to('cpu')
                    # pred=torch.clamp(pred, 0.0, 1.0)
                    if conditional:

                        reali = inverse_data_transform(self.config, test_X.to('cpu'))
                        condi = inverse_data_transform(self.config, test_cond.to('cpu'))
                        # reali = torch.clamp(test_X, 0.0, 1.0).to('cpu')
                        # condi = torch.clamp(test_cond, 0.0, 1.0).to('cpu')
                        if future > 0:
    ...
    def sample_traj(self):
        ...
        if self.config.sampling.ckpt_id is None:
            ckpt = "latest"
            states = torch.load(os.path.join(self.args.log_path, 'checkpoint_160000.pt'), map_location=self.config.device)
        else:
            ckpt = self.config.sampling.ckpt_id
            states = torch.load(os.path.join(self.args.log_path, f'checkpoint_{ckpt}.pt'),
                                map_location=self.config.device)
            # states =   torch.load(os.path.join("/opt/data/private/mcvd-pytorch/teacher_pths", f'checkpoint_62000.pt'),
            #     map_location=self.config.device)
        scorenet = get_stu_model(self.config)
        # self.vae=scorenet.first_stage_model.to(self.config.device)
        scorenet = torch.nn.DataParallel(scorenet)
        ...
        if self.config.data.dataset.upper() == 'FFHQ':
            ...
            dataloader = DataLoader(dataset, batch_size=self.config.sampling.batch_size, shuffle=False, num_workers=self.config.data.num_workers)
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                # 负责将生成的样本图像进行后处理和保存。它的主要逻辑是根据配置中的 final_only 参数，决定是保存所有生成的中间样本，还是仅保存最后的生成结果 
                ...
                if self.config.sampling.data_init or conditional:
                    ...
                    real_era5 = real_era5.to(self.config.device)
                    real_era5_3d = real_era5_3d.to(self.config.device)

                    gt = real.clone()

                    for i in range(gt.size(0)):
                        # for j in range(real.size(1)):
                        gt[i]=gt[i]*(min_max['max'][i]-min_max['min'][i])+min_max['min'][i]
                    np.save(f'/opt/data/private/mcvd-pytorch2/mcvd-pytorch/{save_image_name}-gt.npy',gt.cpu().detach().numpy())
                    real = real[:,:9]
                    real = data_transform(self.config, real)

                    real_era5 = real_era5[:,:9]
                    real_era5 = data_transform(self.config, real_era5)
                    
                    real_era5_3d = real_era5_3d[:,:9]
                    real_era5_3d = data_transform(self.config, real_era5_3d)

                    real, cond, cond_mask = conditioning_fn(self.config, real, num_frames_pred=self.config.data.num_frames, conditional=conditional)
                    _, cond_era5, _ = conditioning_fn2(self.config, real_era5, num_frames_pred=self.config.data.num_frames, conditional=conditional)
                    _, cond_era5_3d, _ = conditioning_fn3(self.config, real_era5_3d, num_frames_pred=self.config.data.num_frames, conditional=conditional)
                # z
                init_samples_shape = (self.config.sampling.batch_size, self.config.data.channels*self.config.data.num_frames,
                                      self.config.data.image_size, self.config.data.image_size)
                if self.version == "SMLD":
                ...
                if self.config.sampling.data_init:
                    ...
                    init_samples = z.float()

                # Sampler
                sampler = self.get_sampler()
                all_samples = None
                traj=[cond]
                for j in range(cond.size(1)): # 4
                    if all_samples is not None:
                        cond = torch.cat([cond[:,1:],all_samples[0,:,0:1]],dim=1)

                    all_samples = sampler(init_samples, scorenet, cond=cond.float(), cond_mask=cond_mask,cond_era5=cond_era5.float(),cond_era5_3d=cond_era5_3d.float(),attn=True,
                                        n_steps_each=self.config.sampling.n_steps_each,
                                        ...
                                        subsample_steps=getattr(self.config.sampling, 'subsample', None),
                                        clip_before=getattr(self.config.sampling, 'clip_before', True),
                                        log=True, gamma=getattr(self.config.model, 'gamma', False))

                    if j == cond.size(1)-1:# 3     # 1,84,5,128,128
                        traj.append(all_samples[0])
                    else:
                        traj.append(all_samples[0,:,0:1])
                sample=torch.cat(traj,dim=1)
                if not self.config.sampling.final_only:
                    for i, sample in tqdm(enumerate(all_samples), total=len(all_samples),
                                               desc="saving image samples"):
                        sample = sample.reshape(sample.shape[0], self.config.data.channels,
                    ...
                    for i in range(real.size(0)):
                        # for j in range(real.size(1)):
                        sample[i]=sample[i]*(min_max['max'][i]-min_max['min'][i])+min_max['min'][i]
                   
                    # np.save(f'/opt/data/private/mcvd-pytorch/TCGdiff_pred_traj-24-3-lx+era5-{save_image_name}.npy',sample.cpu().detach().numpy())
                    np.save(f'/opt/data/private/mcvd-pytorch2/mcvd-pytorch/{save_image_name}.npy',sample.cpu().detach().numpy())
                 
                    
             
                    
    def sample_traj_lx(self,ckpt=None):
        ...
        # Future
        future = getattr(self.config.data, "num_frames_future", 0)
        save_image_name = f"????-encoder-distill-{ckpt}"
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                data_iter = iter(dataloader)
                refer_images, _ = next(data_iter)
                ...
                if self.config.sampling.data_init or conditional:
                    ...
                    for i in range(gt.size(0)):
                        # for j in range(real.size(1)):
                        gt[i]=gt[i]*(min_max['max'][i]-min_max['min'][i])+min_max['min'][i]
                    np.save(f'/opt/data/private/mcvd-pytorch/{save_image_name}-gt.npy',gt.cpu().detach().numpy())
                    real = real[:,:9]
                    real = data_transform(self.config, real)
                ...
                if not self.config.sampling.final_only:
                    ...
                    # np.save(f'/opt/data/private/mcvd-pytorch/TCGdiff_pred_traj-24-3-lx+era5-{save_image_name}.npy',sample.cpu().detach().numpy())
                    np.save(f'/opt/data/private/mcvd-pytorch/{save_image_name}.npy',sample.cpu().detach().numpy())
        torch.cuda.empty_cache()
        del scorenet, net, states
        gc.collect()
    ...
    def sample(self):
        if self.config.sampling.ckpt_id is None:
            ckpt = "latest"
            states = torch.load(os.path.join(self.args.log_path, 'checkpoint_53000.pt'), map_location=self.config.device)
        else:
            ckpt = self.config.sampling.ckpt_id
            states = torch.load(os.path.join(self.args.log_path, f'checkpoint_{ckpt}.pt'),
                                map_location=self.config.device)
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                all_samples = sampler(init_samples, scorenet, cond=cond.float(), cond_mask=cond_mask,
                                      n_steps_each=self.config.sampling.n_steps_each,
                                      step_lr=self.config.sampling.step_lr, verbose=True,
                                      final_only=self.config.sampling.final_only,
                                      denoise=self.config.sampling.denoise,
                ...
                cond = inverse_data_transform(self.config, cond)
                if not self.config.sampling.final_only:
                    for i, sample in tqdm(enumerate(all_samples), total=len(all_samples),
                                               desc="saving image samples"):
                        sample = sample.reshape(sample.shape[0], self.config.data.channels,
                    ...
                    sample = inverse_data_transform(self.config, sample)
                    # sample=self.decode_first_stage(sample.float())
                    # sample = torch.clamp(sample, 0.0, 1.0).to(real.device)
                    
                    sample_bchw = rearrange(sample, "b t h w -> (b t) h w").unsqueeze(1)
                    real_bchw = rearrange(real, "b t h w -> (b t) h w").unsqueeze(1)
                    sample_fvd = sample.unsqueeze(-1)
                    real_fvd = real.unsqueeze(-1)
                    ssim = self.test_ssim(sample_bchw,real_bchw)
                    self.test_fvd.update(sample_fvd.float(), real=False)
                    self.test_fvd.update(real_fvd.float(), real=True)
                    for i in range(real.size(0)):
                        # for j in range(real.size(1)):
                        sample[i]=sample[i]*(min_max['max'][i]-min_max['min'][i])+min_max['min'][i]
                        real[i]=real[i]*(min_max['max'][i]-min_max['min'][i])+min_max['min'][i]
                        cond[i]=cond[i]*(min_max['max'][i]-min_max['min'][i])+min_max['min'][i]
                    np.save('/opt/data/private/mcvd-pytorch/TCGdiff_pred_nonauto.npy',sample.cpu().detach().numpy())
                    # np.save('/opt/data/private/mcvd-pytorch/TCGdiff_gt_1.npy',real.cpu().detach().numpy())
                    # np.save('/opt/data/private/mcvd-pytorch/TCGdiff_cond_1.npy',cond.cpu().detach().numpy())
                    self.valid_mae(sample.contiguous(),real.contiguous())
                    self.valid_mse(sample.contiguous(),real.contiguous())
                    print("mse",self.valid_mse.compute())
                    print("mae",self.valid_mae.compute())
                    print("ssim",self.test_ssim.compute())
                    print("fvd",self.test_fvd.compute())
                    # Stretch out multiple frames horizontally
                ...
                if conditional:
                    ...
                    real = stretch_image(real, self.config.data.channels, self.config.data.image_size)
                    if future > 0:
                    ...
                    cond = stretch_image(cond, self.config.data.channels, self.config.data.image_size)
    ...
    #     z = rearrange(z, "N (T C) H W -> (N T) C H W",N=batch_size,C=self.config.data.channels)
    #     output = self.vae.decode(z)
    #     if isinstance(output, DecoderOutput):
    #         output = output.sample
    #     output = rearrange(output, "(N T) C H W -> N T C H W", N=batch_size).squeeze(2)
    #     return output
    @torch.no_grad()
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        def my_collate(batch):
            data, _ = zip(*batch)  # 使用 zip(*batch) 将批次中的数据和标签解压成两个单独的列表。data 包含所有的数据，而 _ （下划线表示不关心的变量）包含所有的标签。在这里，标签被忽略，因为只对数据进行处理。
            
            data1=[item[0] for item in data]
            data2=[item[1] for item in data]
            # print("data1.shape",data1.shape,data2.shape)
            # data2 = data[1][]
            data1 = torch.stack(data1).repeat_interleave(preds_per_test, dim=0)   # 使用 torch.stack(data) 将 data 列表中的张量堆叠在一起，形成一个更高维的张量。 
            data2 = torch.stack(data2).repeat_interleave(preds_per_test, dim=0)  
            print(data1.shape)
            print(data2.shape)                                                                # repeat_interleave(preds_per_test, dim=0) 将堆叠后的张量在第一个维度上重复 preds_per_test 次。这个操作的目的是将每个样本的数据复制多次，以便在后续处理时可以生成多个预测。
            return (data1,data2), torch.zeros(len(data))
        ...
        for i, (Real_, _) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)), desc="\nvideo_gen dataloader"):
            ...
            real_ = data_transform(self.config, Real_[0]).float()
            real_era5 = data_transform(self.config, Real_[1]).float()
            # real_ = real_.float()
            ...
            real2, cond_era5, _ = conditioning_fn2(self.config, real_era5, num_frames_pred=num_frames_pred,
                                                    prob_mask_cond=0.0, prob_mask_future=0.0, conditional=conditional)
            real2 = inverse_data_transform(self.config, real2)
            cond_original_era5 = inverse_data_transform(self.config, cond_era5.clone())
            cond_era5 = cond_era5.to(self.config.device)
            # z
            ...
            for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                ...
                gen_samples = sampler(init_samples if i_frame==0 or getattr(self.config.sampling, 'init_prev_t', -1) <= 0 else gen_samples,
                                      mynet, cond=cond, cond_mask=cond_mask,cond_era5=cond_era5,
                ...
                if self.config.sampling.data_init:
                    ...
                    init_samples = z
            # pred_samples=self.decode_first_stage(pred_samples[0]).to('cpu')
            pred = torch.cat(pred_samples, dim=1)[:, :self.config.data.channels*num_frames_pred]
            pred = inverse_data_transform(self.config, pred)
            # pred = torch.clamp(pred_samples,0.0,1.0)
