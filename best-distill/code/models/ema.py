...
class EMAHelper(object):
    ...
    def ema_copy(self, module):
        if isinstance(module, nn.DataParallel):
            ...
            module_copy = type(inner_module)(inner_module.config, is_stu=inner_module.is_stu).to(inner_module.config.device)
            module_copy.load_state_dict(inner_module.state_dict())
            module_copy = nn.DataParallel(module_copy)
        else:
            module_copy = type(module)(module.config, is_stu=inner_module.is_stu).to(module.config.device)
