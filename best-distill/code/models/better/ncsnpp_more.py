...
class NCSNpp_stu(nn.Module):
  ...
  def forward(self, x, time_cond, cond=None, cond_mask=None,target=None,return_feat=None):
    self.channel_visual_info = {}

    cond_data = rearrange(cond.unsqueeze(2), 'b f c h w -> b c f h w')

    casual_loss=0
    if 'sst' in self.era5_compositions:
      ### DropPath mask
      sst_32 = self.sst_embedding1(cond_data) # b,384,32,32
      sst_16 = self.sst_embedding2(cond_data) # b,384,16,16
      sst_32 = sst_32.squeeze(2) # B, 384, 32, 32
      sst_16 = sst_16.squeeze(2) # B, 384, 16, 16
    if 'msl' in self.era5_compositions:
      ### DropPath mask
      msl_32 = self.msl_embedding1(cond_data) # b,384,32,32
      msl_16 = self.msl_embedding2(cond_data) # b,384,16,16
      msl_32 = msl_32.squeeze(2) # B, 384, 32, 32
      msl_16 = msl_16.squeeze(2) # B, 384, 16, 16
    if 'z' in self.era5_compositions:
      ### DropPath mask
      z_32 = self.z_embedding1(cond_data) # b,384,32,32
      z_16 = self.z_embedding2(cond_data) # b,384,16,16
      z_32 = z_32.squeeze(2) # B, 384, 32, 32
      z_16 = z_16.squeeze(2) # B, 384, 16, 16
    if 'r' in self.era5_compositions:
      ### DropPath mask
      r_32 = self.r_embedding1(cond_data) # b,384,32,32
      r_16 = self.r_embedding2(cond_data) # b,384,16,16
    ...
    for i_level in reversed(range(self.num_resolutions)):
      ...
      if i_level == 3:
        ...
        for i, var in enumerate(self.era5_compositions):
          ...
          # self.channel_visual_info[var] = indices[0].detach().cpu().numpy()
          ...
          feat_32[var]=attn_feat
        #   visualize_topk_attn_channels(attn_feat=attn_feat,indices=indices, sample_idx=0,var_name=var,topk=30)
        # visualize_topk_indices(self.channel_visual_info, topk=30)
        h = h+fused_feat
    ...
    if return_feat is True:
      return h,feat_16,feat_32,encoded16,encoded32,casual_loss # mid_feat1 是 16  mid_feat2 是 32
    else:
      return h
...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.gamma  = nn.Parameter(torch.tensor(0.0))
    self.temporalAttention = TemporalSelfAttention(num_frames=9, embed_dim=32*32, downsample_size=32, num_heads=4)
  def forward(self, x, time_cond, cond=None, cond_mask=None,data_2d=None,data_3d=None,target=None,p_i=None,return_feat=None):
    mid_feat1=0.0
    mid_feat2=0.0
    ...
    for i_level in reversed(range(self.num_resolutions)):
      ...
      if i_level == 3:
        ...
        for i, var in enumerate(self.era5_compositions):
          ...
          var_feat = encoded32[var] # var_feat   =  [B,384,16,16]
          attn_feat, attn_weights= self.cross_attns32[var](query=var_feat,context=h) # [B,384,16,16] [B,H,W]
          causal_feat, noncausal_feat, binary_mask,soft_mask  = self.var_causal32[var](attn_feat)  # att_feat：因果掩码处理后的特征   mask_prob：soft 掩码（概率型 α）哪些区域可能是因果的”，也可用于可视化 
                                                                        # hard_mask：离散因果掩码（二值型）控制 feature * hard_mask 和 feature * (1 - hard_mask).detach() 的权重分配；
          if target is not None:
            is_training =True
          else:
    ...
    if return_feat is True:
      return h,feat_16,feat_32,encoded16,encoded32 # mid_feat1 是 16  mid_feat2 是 32
    else:
      return h
...
class UNetMore_DDPM(nn.Module):
  def __init__(self, config,is_stu=False):
    super().__init__()
  ...
  def forward(self, x, y, cond=None, cond_mask=None,era5_cond=None,era5_cond_3d=None,attn=None,target=None,p_i=None):
    ...
    if self.is_stu is False: # 教师模型进这里
        return self.unet(x, y, cond, cond_mask=cond_mask,data_2d = era5_cond,data_3d=era5_cond_3d,p_i=p_i, return_feat=True)
    else: # 学生模型进这里
        if target is not None:
          unet,feat_16,feat_32,encoded16,encoded32,casual_loss = self.unet(x, y, cond, cond_mask=cond_mask,return_feat=True,target=target)
          return unet,feat_16,feat_32,encoded16,encoded32,casual_loss
        else:
          return self.unet(x, y, cond, cond_mask=cond_mask,return_feat=False)
...
class CausalMaskNet_img(nn.Module):
    def __init__(self, in_channels, hidden_channels=384, mask_ratio=0.7):
        super().__init__()
        self.net = nn.Sequential(
    ...
    def forward(self, feat):
        ...
        """
        B, C, H, W = feat.shape
        soft_mask = self.net(feat).squeeze(1)  # B x H x W

        # 计算 Top-k 二值 Mask
        flatten = soft_mask.view(B, -1)
        k = int(self.mask_ratio * flatten.shape[1])
        _, indices = torch.topk(flatten, k, dim=1)

        binary_mask = torch.zeros_like(flatten)
        binary_mask.scatter_(1, indices, 1.0)
        binary_mask = binary_mask.view(B, 1, H, W)
