...
TCGdiff_preds=torch.from_numpy(np.load(f'/opt/data/private/mcvd-pytorch/{save_image_name}.npy')).squeeze(-1).unsqueeze(2).to(torch.device("cuda")).float()[:,4:]# 

# #gt.shape  84,8,1,128,128          lx 的  TCGdiff_gt_traj-24-3-lx+era5-{save_image_name}  wyx的 
gt=torch.from_numpy(np.load(f'/opt/data/private/mcvd-pytorch/{save_image_name}-gt.npy')).squeeze(-1).unsqueeze(2).to(torch.device("cuda")).float()[:,4:]
...
save_image(gt_grid, os.path.join('/opt/data/private/mcvd-pytorch2/mcvd-pytorch/lx', f'gt_full_grid-24+era5-{save_image_name}.png'))

TCGdiff_grid = make_grid(TCGdiff_preds, nrow=nrow, padding=6, pad_value=0.5)
save_image(TCGdiff_grid, os.path.join('/opt/data/private/mcvd-pytorch2/mcvd-pytorch/lx', f'TCGdiff_full_grid-24+era5-{save_image_name}.png'))
