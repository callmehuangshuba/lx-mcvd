...
def anneal_dsm_score_estimation(scorenet, x, labels=None, loss_type='a', hook=None, cond=None, cond_mask=None,era5_cond=None, era5_cond_3d=None,gamma=False, L1=False, all_frames=False):
    ...
    if version == "SMLD":
        ...
        alphas = net.alphas
        if labels is None: # 进这里 labels.shape = B
    ...
    scorenet = partial(scorenet, cond=cond,era5_cond=era5_cond,era5_cond_3d=era5_cond_3d,target=x,used_alphas=used_alphas)
    ...
    if L1:
        ...
        def pow_(x):
            return 1 / 2. * x.square()
    predict,causal_loss,grad_loss = scorenet( perturbed_x, labels, cond_mask=cond_mask)
    loss1 = pow_((z - predict).reshape(len(x), -1)).sum(dim=-1).mean(dim=0)
    epsilon = 1e-6
    max_lambda = 100
    min_lambda = 0.01

    lambda_casual = loss1.item() /4* (abs(causal_loss.item()) + epsilon)
    # lambda_grad = loss1.item() /4* (abs(grad_loss.item()) + epsilon)
    
    lambda_adaptive_casual = max(min_lambda, min(lambda_casual, max_lambda)) 
    # lambda_adaptive_grad = max(min_lambda, min(lambda_grad, max_lambda)) 


    loss = loss1+lambda_adaptive_casual*causal_loss+10*grad_loss
