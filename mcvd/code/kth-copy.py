...
class TCGVideoDataset(Dataset):
    ...
    def __getitem__(self, index, time_idx=0):
        ...
        # shard_idx, idx_in_shard = self.videos_ds.get_indices(video_index)
        one_TC_vid_path=self.videos_ds.shard_paths[video_index]
        one_TC_vid_path_era5 = self.videos_ds.shard_paths2[video_index]
        one_TC_vid_path_era5_3d = self.videos_ds.shard_paths3[video_index]
        ...
        frames_list_h5_2d=sorted([os.path.join(one_TC_vid_path_era5,x) for x in os.listdir(one_TC_vid_path_era5) if x.endswith('.npy')])
        frames_list_h5_3d=sorted([os.path.join(one_TC_vid_path_era5_3d,x) for x in os.listdir(one_TC_vid_path_era5_3d) if x.endswith('.npy')])
        num_frames=len(frames_list_h5)
        # num_frames=len(one_TC_h5_list)
        # sample frames
        if self.random_time and num_frames > self.num_frames:
            # sampling start frames
            time_idx = np.random.choice(num_frames - (self.num_frames-1)*self.time_resolution)
        # time_idx=12
        frame_files=frames_list_h5[time_idx:min(time_idx + self.num_frames*self.time_resolution, num_frames):self.time_resolution]
        frame_files2=frames_list_h5_2d[time_idx:min(time_idx + self.num_frames*self.time_resolution, num_frames):self.time_resolution]
        frame_files3=frames_list_h5_3d[time_idx:min(time_idx + self.num_frames*self.time_resolution, num_frames):self.time_resolution]

        with ThreadPoolExecutor(max_workers=12) as executor:
            prefinals = list(executor.map(self.read_frames, frame_files))
        with ThreadPoolExecutor(max_workers=12) as executor:
            prefinals2 = list(executor.map(self.read_frames2, frame_files2))
        with ThreadPoolExecutor(max_workers=12) as executor:
            prefinals3 = list(executor.map(self.read_frames3, frame_files3))
