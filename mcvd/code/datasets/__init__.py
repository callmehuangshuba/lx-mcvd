...
from datasets.kth import KTHDataset,TCGVideoDataset
from datasets.cityscapes import CityscapesDataset
from datasets.ucf101 import UCF101Dataset
from torch.utils.data import Subset


DATASETS = ['TCG','CIFAR10', 'CELEBA', 'LSUN', 'FFHQ', 'IMAGENET', 'MOVINGMNIST', 'STOCHASTICMOVINGMNIST', 'BAIR', 'KTH', 'CITYSCAPES', 'UCF101']
...
def                          get_dataset(data_path, config, video_frames_pred=0, start_at=0):
    ...
    elif config.data.dataset.upper() == "TCG":
        # KTH64_h5 (data_path)
        # |-- shard_0001.hdf5
        frames_per_sample = config.data.num_frames_cond + getattr(config.data, "num_frames_future", 0) + video_frames_pred
 
        # dataset = TCGVideoDataset(time_resolution=3,data_dir=data_path,type='TC_img_npy/TCG_test_traj-3',type2="era5_dataset_npy_84/2D/test",type3="era5_dataset_npy_84/3D/test",num_frames=12)
        # test_dataset = TCGVideoDataset(time_resolution=3,data_dir=data_path,type='TC_img_npy/TCG_test_traj-3',type2="era5_dataset_npy_84/2D/test",type3="era5_dataset_npy_84/3D/test",num_frames=12)   

        dataset = TCGVideoDataset(time_resolution=3,data_dir=data_path,type='TC_img_npy/train_val',type2="era5_dataset_npy/2D/train",type3="era5_dataset_npy/3D/train",num_frames=9)
        test_dataset = TCGVideoDataset(time_resolution=3,data_dir=data_path,type='TC_img_npy/test',type2="era5_dataset_npy/2D/test",type3="era5_dataset_npy/3D/test",num_frames=9)   
