...
default_opener = lambda p_: np.load(p_, allow_pickle=True)
from concurrent.futures import ThreadPoolExecutor
from scipy.ndimage import distance_transform_edt

class TCG_NPYDataset(Dataset):

    @staticmethod
    def _get_num_in_shard(shard_p, opener=default_opener):
        print(f'\rnpy: Opening {shard_p}... ', end='')
        try:
            with opener(shard_p) as f:
                num_per_shard = len(f['len'].keys())
        except:
            print(f"npy: Could not open {shard_p}!")
            num_per_shard = -1
    ...
    def check_shard_lengths(file_paths, opener=default_opener):
        ...
        for i, p in enumerate(file_paths):
            shard_lengths.append(TCG_NPYDataset._get_num_in_shard(p, opener))
        return shard_lengths

    def __init__(self, data_path, data_path2=None, data_path3=None,  # hdf5 file, or directory of hdf5s
        ...
        # If `data_path` is an npy file
        if os.path.splitext(self.data_path)[-1] == '.npy':
            self.data_dir = os.path.dirname(self.data_path)
            self.shard_paths = [self.data_path]
        # Else, if `data_path` is a directory of npy
        else:
            self.data_dir = self.data_path
            self.shard_paths = sorted(glob.glob(os.path.join(self.data_dir, '*.npy')) + glob.glob(os.path.join(self.data_dir, '*')))

            self.data_dir2 = self.data_path2
            self.shard_paths2 = sorted(glob.glob(os.path.join(self.data_dir2, '*.npy')) + glob.glob(os.path.join(self.data_dir2, '*')))

            self.data_dir3 = self.data_path3
            self.shard_paths3 = sorted(glob.glob(os.path.join(self.data_dir3, '*.npy')) + glob.glob(os.path.join(self.data_dir3, '*')))

        assert len(self.shard_paths) > 0, "npy: Directory does not have any .npy files! Dir: " + self.data_dir

        assert len(self.shard_paths2) > 0, "npy: Directory does not have any .npy files! Dir: " + self.data_dir2

        assert len(self.shard_paths3) > 0, "npy: Directory does not have any .npy files! Dir: " + self.data_dir3
        # self.shard_lengths = HDF5Dataset.check_shard_lengths(self.shard_paths, self.opener)
        self.shard_lengths = [len(self.shard_paths)]
        self.num_per_shard = self.shard_lengths[0]
        self.total_num = sum(self.shard_lengths)

        assert len(self.shard_paths) > 0, "npy: Could not find .npy files! Dir: " + self.data_dir + " ; len(self.shard_paths) = " + str(len(self.shard_paths))

        self.num_of_shards = len(self.shard_paths)

        print("npy: paths", len(self.shard_paths), "; shard_lengths", self.shard_lengths, "; total", self.total_num)

        # self.total_TC_vid_list=[]
        # for one_TC_vid_path in self.shard_paths:
        #     frames_list_h5=sorted([os.path.join(one_TC_vid_path,x) for x in os.listdir(one_TC_vid_path) if x.endswith('.npy')])
        #     one_TC_h5_list=[]
        #     with ThreadPoolExecutor(max_workers=16) as executor:
        #         one_TC_h5_list = list(executor.map(self.read_frames, frames_list_h5))
        #     self.total_TC_vid_list.append(one_TC_h5_list)
        #     print(one_TC_vid_path,"已加入total_TC_vid_list")
            
        # self.total_era5_vid_list=[]
        # for one_era5_vid_path in self.shard_paths2:
        #     frames_list_era5_h5=sorted([os.path.join(one_era5_vid_path,x) for x in os.listdir(one_era5_vid_path) if x.endswith('.npy')])
        #     one_era5_h5_list=[]
        #     with ThreadPoolExecutor(max_workers=16) as executor:
        #         one_era5_h5_list = list(executor.map(self.read_frames2, frames_list_era5_h5))
        #     self.total_era5_vid_list.append(one_era5_h5_list)
        #     print(one_era5_vid_path,"已加入total_era5_vid_list")

        # self.total_era5_3d_vid_list=[]
        # for one_era5_vid_path in self.shard_paths3:
        #     frames_list_era5_3d_h5=sorted([os.path.join(one_era5_vid_path,x) for x in os.listdir(one_era5_vid_path) if x.endswith('.npy')])
        #     one_era5_3d_h5_list=[]
        #     with ThreadPoolExecutor(max_workers=16) as executor:
        #         one_era5_3d_h5_list = list(executor.map(self.read_frames3, frames_list_era5_3d_h5))
        #     self.total_era5_3d_vid_list.append(one_era5_3d_h5_list)
        #     print(one_era5_vid_path,"已加入total_era5_vid_list")
        # Shuffle shards 
    ...
    def read_frames(self, frame_name):
        arr = np.load(frame_name)
        arr = torch.from_numpy(arr) # 转换为 PyTorch Tensor
        return arr
    def read_frames2(self, frame_name):
        data = np.load(frame_name, allow_pickle=True).item()
        mslp = data["msl"]
        sst = data['sst']
        sst = np.nan_to_num(sst, nan=0.0)
        return torch.from_numpy(np.vstack((sst, mslp)))

    def read_frames3(self, frame_name):
        data = np.load(frame_name, allow_pickle=True).item()
        z = data["z"]
        r = data['r']
        vo = data['vo']
        return torch.from_numpy(np.vstack((z, r,vo)))
...
class HDF5Dataset(Dataset):
    ...
    def __init__(self, data_path,   # hdf5 file, or directory of hdf5s
        ...
        # Else, if `data_path` is a directory of hdf5s
            ...
            self.data_dir = self.data_path
            self.shard_paths = sorted(glob.glob(os.path.join(self.data_dir, '*.hdf5')) + glob.glob(os.path.join(self.data_dir, '*')))
