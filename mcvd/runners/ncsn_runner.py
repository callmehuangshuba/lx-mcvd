...
class NCSNRunner():
    ...
    def train(self):
        ...
        if self.args.resume_training:
            # states = torch.load(os.path.join(self.args.log_path, 'checkpoint_10000.pt'))
    ...
    def sample_traj(self):
        ...
        # Future
        future = getattr(self.config.data, "num_frames_future", 0)
        save_image_name = "new-ste-104000"
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                if self.config.sampling.data_init or conditional:
                    ...
                    np.save(f'/opt/data/private/mcvd-pytorch3/mcvd-pytorch/{save_image_name}-gt.npy',gt.cpu().detach().numpy())
                    real = real[:,:9]
                ...
                if not self.config.sampling.final_only:
                    ...
                    np.save(f'/opt/data/private/mcvd-pytorch3/mcvd-pytorch/{save_image_name}.npy',sample.cpu().detach().numpy())
