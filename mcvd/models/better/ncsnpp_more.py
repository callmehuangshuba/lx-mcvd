...
# from lx_visuals import visualize_attention16,visualize_attention32,visualize_attention128,visualize_denoising,visualize_attention_and_mask
# from lx_losses import compute_image_gradient,gradient_loss
# from lx_modules import GradientEncoder,CrossAttention_Casual, CrossAttention,SPADE_NCSNpp, CausalMaskNet, CausalEffectMatrix,ConditionalMutualInfoLoss,TemporalSelfAttention,disabled_train,Transformer_v2,PreNormattention,Attention,EmbedFC,ResidualConvBlock
from models.better.lx_visuals import visualize_topk_indices,visualize_topk_attn_channels,visualize_aug_diff_triplet
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.grad_encoder = GradientEncoder()
    self.grad_embedding1 =   nn.Sequential(
          # 输入: (B, 1, 4, 128, 128)   (B , C, T, H, W）
        nn.Conv3d(1, 96, kernel_size=(2, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)),  # -> B, 96, 3, 64, 64
        nn.SiLU(),
        nn.Conv3d(96, 192, kernel_size=(2, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)), # -> B, 192, 2, 32, 32
    ...
    # self.grad_embedding2 =   nn.Sequential(
    #       # 输入: (B, 1, 4, 128, 128)   (B , C, T, H, W）
    #     nn.Conv3d(1, 96, kernel_size=(2, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)),  # -> B, 96, 3, 64, 64
    #     nn.SiLU(),
    #     nn.Conv3d(96, 192, kernel_size=(2, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)), # -> B, 192, 2, 32, 32
    ...
    #     nn.SiLU()
    #   )
    # self.grad_attns16 = CrossAttention(dim=384)
    self.grad_attns32 = CrossAttention(dim=384)
    ...
    self.grad = True
  def forward(self, x, time_cond, cond=None, cond_mask=None,data_2d=None,data_3d=None,target=None,p_i=None,used_alphas=None):
    noise_x= x
    ...
    # visualize_and_save_era5_variables(sst,msl,z,r)
    vo=None
    casual_loss= torch.tensor(0.0, device=x.device)
    grad_loss = torch.tensor(0.0, device=x.device)

    # visualize_data(sst,msl,z,r,grad_embedding.unsqueeze(2),cond)  
    if sst is not None:
      ### DropPath mask
    ...
    # 梯度emb
    grad_embedding = self.grad_encoder(cond)
    grad_data = rearrange(grad_embedding.unsqueeze(2), 'b f c h w -> b c f h w')
    grad_32 = self.grad_embedding1(grad_data).squeeze(2)
    # grad_16 = self.grad_embedding2(grad_data).squeeze(2)
    ...
    #   B,C,H,W=grad_16.shape
    #   var_feat = grad_16   # var_feat   =  [B,384,16,16]
    #   attn_feat, attn_weights = self.grad_attns16(query=var_feat,context=h) # [B,384,16,16] [B,H,W]
    #   fused_feat += attn_feat
    #   self.attention_maps16['grad'] = attn_weights.detach() 
    h = h+fused_feat
    ...
    for i_level in reversed(range(self.num_resolutions)):
      ...
      if i_level == 3:
        ...
        # visualize_data(data_2d=data_2d,data_3d=data_3d,cond = cond,save_dir="/opt/data/private/mcvd-pytorch2/mcvd-pytorch/lx_2d+3d_era5_attn_visual/era5_cond_visual_6.3.png")
        indices_dict={}
        for i, var in enumerate(self.era5_compositions):
          ...
          causal_feat, noncausal_feat, binary_mask,indices  = self.var_causal32[var](attn_feat)  # att_feat：因果掩码处理后的特征   mask_prob：soft 掩码（概率型 α）哪些区域可能是因果的”，也可用于可视化 
                                                                        # hard_mask：离散因果掩码（二值型）控制 feature * hard_mask 和 feature * (1 - hard_mask).detach() 的权重分配；
          if target is not None:
            is_training =True
          else:
            is_training = False
          enhance_causal_feat = self.causal_effect(causal_feat,noncausal_feat,target=target,var_index=self.era5_compositions.index(var),is_training=is_training)                     

          if target is not None:
            cmi_loss = self.cmi_criterion(enhance_causal_feat, noncausal_feat, target)
            casual_loss+=cmi_loss
          fused_feat += (enhance_causal_feat+noncausal_feat)*weights[i]
          indices_dict[var] = indices
          # visualize_topk_attn_channels(attn_feat=attn_feat,indices=indices, sample_idx=0,var_name=var,topk=10)
          # visualize_aug_diff_triplet(causal_feat,enhance_causal_feat,indices,var_name=var)
          self.attention_maps32[var] = attn_weights.detach()  # 平均多头权重  36 H W
        if self.grad is True:
          var_feat = grad_32   # var_feat   =  [B,384,16,16]
          attn_feat, attn_weights = self.grad_attns32(query=var_feat,context=h) # [B,384,32,32] [B,H,W]
          fused_feat += attn_feat
        h = h+fused_feat
        # visualize_topk_indices(indices_dict)
    ...
    if target is not None:
      ...
      # 恢复预测图像 x0_pred  计算梯度loss
      alpha_t = used_alphas  # 形状为 [B, 1, 1, 1] 等
      x0_pred = (noise_x - (1 - alpha_t).sqrt() * h) / alpha_t.sqrt()

      grad_loss = gradient_loss(x0_pred, target, loss_type='l1')
      # grad_loss=0
      # visualize_denoising(noise_x, x0_pred, target, num_samples=5,step=10000)
      return h,casual_loss,grad_loss
    else:
      return h
...
class UNetMore_DDPM(nn.Module):
  ...
  def forward(self, x, y, cond=None, cond_mask=None,era5_cond=None,era5_cond_3d=None,attn=None,target=None,p_i=None,used_alphas=None):
    ...
    if target is not None:
       unet,causal_loss,grad_loss = self.unet(x, y, cond, cond_mask=cond_mask,data_2d = era5_cond,data_3d=era5_cond_3d,target=target,used_alphas=used_alphas)
       return unet,causal_loss,grad_loss
    else:
...
class CausalMaskNet(nn.Module):
    ...
    def forward(self, feat):
        ...
        binary_mask.scatter_(1, indices, 1.0)
        # STE 梯度传播
        binary_mask = binary_mask + (soft_mask - soft_mask.detach())
        ...
        return causal_feat, noncausal_feat, binary_mask,indices
...
class CausalEffectMatrix(nn.Module):
    ...
    def forward(self, causal_feat,noncausal_feat, target, var_index, is_training=True):
        """
        计算因果效应并增强特征
        ...
        if is_training:
            ...
            factual_pred = self.dce_net(causal_feat+noncausal_feat)  # [B, target_channels=5]
            
            # 反事实干预 - 将特征置零
            counterfactual_pred = self.dce_net(noncausal_feat)  # [B, target_channels=5]
        ...
        # 对特征进行因果增强
        B, C, H, W = causal_feat.shape
        effect_weights = var_effect.view(1, -1, 1, 1)  # [ 1, target_channels=5, 1, 1]
        
        # 通过1x1卷积将特征映射到target_channels维度
        feat_proj = self.proj_layer(causal_feat)
        # 应用因果增强
        enhanced_feat = causal_feat + (feat_proj * effect_weights).sum(dim=1, keepdim=True)
...
def visualize_data(sst,msl,z,r,grad, cond, save_dir='visualizations', batch_idx=0, sample_num=9):
    ...
    for i in range(min(sample_num, 9)):
        ...
        for t in range(4):
            ...
            axes[4, t].imshow(grad[i, t, 0].cpu().numpy(), cmap='gray')
            axes[4, t].set_title(f'Grad T{t}')
            axes[5, t].imshow(cond_vis[i, t, 0].detach().cpu().numpy(), cmap='jet')
            axes[5, t].set_title(f'Cond T{t}')
        ...
        plt.savefig(os.path.join(save_dir, f'grad-batch{batch_idx}_sample{i}.png'))
        plt.close()
...
def visualize_and_save_era5_variables(sst, msl, z, r, save_dir="/opt/data/private/mcvd-pytorch3/mcvd-pytorch/lx_2d+3d_era5_attn_visual/!!!!!!!.png"):
    os.makedirs(save_dir, exist_ok=True)

    def plot_5ch(tensor, var_name):
        """
        tensor: shape [B, 4, 1, H, W]
        var_name: e.g., 'sst'
        """
        fig, axes = plt.subplots(1, 4, figsize=(12, 3))
        for i in range(4):
            img = tensor[0, i, 0].cpu().numpy()
            axes[i].imshow(img, cmap='viridis')
            axes[i].set_title(f"{var_name} ch{i}")
            axes[i].axis('off')
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"{var_name}.png"))
        plt.close()
...
def visualize_denoising(noise_x, x0_pred, target, num_samples=5, save_path="/opt/data/private/mcvd-pytorch2/mcvd-pytorch/denoise_img",step=None):
  """
  ...
  for i in range(num_samples):
      ...
      plt.tight_layout()
      if save_path:
          plt.savefig(f"{save_path}/sample_{i}_{step}.png")
      else:
          plt.show()
      plt.close()